...
import { Save, Download, CheckCircle, FileText, AlertCircle, Send, PenLine } from 'lucide-react';
...
import { OrganizationSection } from '../components/form/OrganizationSection';
import { PrimaryContactSection } from '../components/form/PrimaryContactSection';
import { ProgramDetailsSection } from '../components/form/ProgramDetailsSection';
import { ChampionSection } from '../components/form/ChampionSection';
import { PricingSection } from '../components/form/PricingSection';
import { SignatorySection } from '../components/form/SignatorySection';
...
const requiredFields = {
  partnerOrgName: 'Organization Legal Name', partnerEntityType: 'Entity Type',
  orgType: 'Organization Type', partnerAddress: 'Street Address', partnerCity: 'City',
  partnerState: 'State', partnerZip: 'ZIP Code', contactName: 'Contact Full Name',
  contactTitle: 'Contact Title', contactEmail: 'Contact Email', contactPhone: 'Contact Phone',
  startDate: 'Start Date', endDate: 'End Date', numOfficials: 'Number of Officials',
  championName: 'Champion Full Name', championTitle: 'Champion Title',
  championEmail: 'Champion Email', championPhone: 'Champion Phone',
  perUserRate: 'Per-User Monthly Rate', signerName: 'Signer Full Name',
  signerTitle: 'Signer Title', signatureDate: 'Signature Date',
};
...
Total Agreement Value: $${calculateTotal(formData.perUserRate, formData.numOfficials, formData.termStructure, formData.pilotDiscount)}
...
`;
  ...
  const generatePDF = () => {
    ...
    const total = calculateTotal(formData.perUserRate, formData.numOfficials, formData.termStructure, formData.pilotDiscount);
    ...
    const addSection = (title, items) => {
      ...
      items.forEach(([label, value]) => {
        ...
        doc.setFont('helvetica', 'normal'); doc.text(value || '[Not provided]', margin + 50, y);
        y += 6;
      });
      y += 6;
    };

    doc.setTextColor(0, 61, 122); doc.setFontSize(10);
    doc.text(`Effective Date: ${formData.signatureDate || '[Date]'}`, margin, y); y += 15;
    ...
    addSection('PARTNER CHAMPION', [
      ['Name', formData.championName], ['Title', formData.championTitle],
      ['Email', formData.championEmail], ['Phone', formData.championPhone],
    ]);
    addSection('PRIMARY CONTACT', [
      ['Name', formData.contactName], ['Title', formData.contactTitle],
      ['Email', formData.contactEmail], ['Phone', formData.contactPhone],
    ]);
    addSection('AUTHORIZED SIGNATORY', [
      ['Name', formData.signerName], ['Title', formData.signerTitle], ['Date', formData.signatureDate],
    ]);

    if (y > 240) { doc.addPage(); y = 20; }
    y += 10;
    doc.setTextColor(100, 100, 100); doc.setFontSize(8);
    doc.text('Signature lines:', margin, y); y += 12;
    ...
    y += 20; doc.setFontSize(7); doc.setTextColor(150, 150, 150);
  ...
  const handleSubmit = async () => {
    if (!validate()) {
      ...
      if (firstErrorField) document.getElementById(firstErrorField)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      return;
    }
    if (!signatureData) {
  ...
  const sectionProps = { formData, errors, handleChange, errorBorder };
    ...
    <section id="partnership-form" data-testid="partnership-form-section" className="section-padding bg-gray-50">
      <div className="max-w-6xl mx-auto container-padding">
        ...
        <AnimatePresence>
          {submitStatus && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              className={`max-w-4xl mx-auto mb-6 p-4 rounded-lg flex items-start gap-3 ${submitStatus === 'success' ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}
            ...
            <Card className="shadow-lg border-iwhistle-blue/10">
              <CardHeader className="bg-gradient-to-r from-iwhistle-deep to-iwhistle-blue text-white rounded-t-lg">
                <CardTitle className="flex items-center gap-3 text-xl">
                  <FileText className="w-6 h-6" /> Partnership Agreement Form
                </CardTitle>
              </CardHeader>
              <CardContent className="p-6 lg:p-8">
                <form className="space-y-8" data-testid="partnership-agreement-form" onSubmit={(e) => e.preventDefault()}>
                  ...
                  <Separator />
                  <PrimaryContactSection {...sectionProps} />
                  <Separator />
                  <ProgramDetailsSection {...sectionProps} />
                  <Separator />
                  <ChampionSection {...sectionProps} />
                  <Separator />
                  <PricingSection {...sectionProps} />
                  <Separator />
                  <SignatorySection {...sectionProps} />
                  <Separator />
                  ...
                  <div id="signature-section">
                    ...
                    <p className="text-sm text-gray-500 mb-4">Draw your signature below to authorize this partnership agreement.</p>
                    <SignaturePad onChange={setSignatureData} hasError={!!errors.signature} />
                    <AnimatePresence>
                      {errors.signature && (
